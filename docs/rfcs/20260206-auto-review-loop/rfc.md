...
- AI 同士のレビュー往復を自動化し、人間の手動操作を「元ネタ入力 → RFC PR マージ → タスク計画承認 → 実装 PR マージ」の4回に削減する
...
```
Phase 1: 実装
  → Task(Implementer): RFC読み込み + タスク計画提示
  ← ユーザがタスク計画を承認（人間チェックポイント）
  → Task(Implementer): 実装 + テスト + コミット + Draft PR作成
...
**`/aimp` の Phase 1 における人間チェックポイント**: `/imp` と同様、実装タスク計画をユーザに提示し承認を得てから実装に着手する。この時点でオーケストレータはユーザの応答を待つ。

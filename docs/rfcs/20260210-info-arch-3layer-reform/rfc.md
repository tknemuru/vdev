...
|---|------|-----------------------------|
| G1 | `adapters/claude/CLAUDE.md` の System Overview Documents セクションを3層情報アーキテクチャに沿って改定し、各ドキュメントの記載内容ガイドラインを「ドメイン知識中心」に修正する | 変更後の `adapters/claude/CLAUDE.md` を Read で読み込み、(1) ドメイン知識中心の記載ガイドラインが定義されていること、(2) 「記載内容の判断基準」テーブルで docs に書く/書かない情報の基準が明示されていること、(3) 技術的事実はコードから導出する方針が明記されていることを確認する |
| G2 | `adapters/claude/commands/rfc.md` の Step 5 を「ドメイン知識は docs から、技術的事実はコードから」の調査方針に変更する | 変更後の `adapters/claude/commands/rfc.md` を Read で読み込み、(1) Step 5 が 5a/5b/5c の3段階構成であること、(2) 5b でコードが唯一の真実である旨が記載されていること、(3) docs の記載を無条件に信頼しない旨の注意が含まれていることを確認する |

#### Verification の設計根拠

本 RFC の成果物は Markdown 定義ファイル2つの文言修正であり、成果物自体にランタイムの振る舞いが存在しない。そのため、Verification には以下の構造的制約がある。

**完全なトートロジー回避が実現できない理由**: 定義ファイルの変更が実際に AI の行動変容をもたらすかは、変更後のファイルを用いて `/rfc` を実行し AI の調査行動を観察することでしか検証できない。しかし、この検証は `/imp` の実装フロー内では実行不可能である（別プロジェクトでの `/rfc` 実行という実装スコープ外の操作が必要になる）。ランタイム振る舞いを持たない定義ファイルの変更において、「成果物の内容確認」と「効果の確認」の間のギャップは原理的に `/imp` 内では埋められない。

**提案の検証で品質担保として十分である理由**: 上記の Verification は、Goal の文言を言い換えただけの自己参照ではなく、詳細設計（5.2/5.3）で定義した具体的な変更ポイント（判断基準テーブルの存在、3段階構成、「コードが唯一の真実」原則の明記等）が正しく反映されているかを個別に確認する手順である。検証項目が詳細設計の仕様に紐づいているため、Goal と Verification の間に詳細設計という中間層が介在し、トートロジーの自己参照構造を断ち切っている。定義ファイルの変更においては、仕様通りの内容が正確に記述されていることの確認が `/imp` 内で達成可能な最大限の品質保証である。
...
- 検証方法: 変更後の各ファイルの内容を Read で読み込み、Goals テーブルの Verification に記載した手順で確認する。
  - `adapters/claude/CLAUDE.md`: ドメイン知識中心の記載ガイドライン、「記載内容の判断基準」テーブル、技術的事実はコードから導出する方針の存在を確認する。
  - `adapters/claude/commands/rfc.md`: Step 5 の3段階構成（5a/5b/5c）、コードが唯一の真実である旨の記載、docs を無条件に信頼しない旨の注意の存在を確認する。
- Verification の設計根拠については Goals セクションの「Verification の設計根拠」を参照のこと。

...
abel リポジトリの RFC 5-1（Paper Trading 検証）作成時に重大な設計ミスが6件発生し、PR レビューで全面的な書き直しが必要になった（詳細は `20260208-report-rfc-quality-analysis.md` を参照）。

同報告書はインプット情報（`docs/ops.md` 等）の不備を根本原因として特定したが、本レポートでは **vdev 側のフロー設計にも構造的な改善ポイントがある** という仮説のもと、ワークフロー全体を分析した。

さらに、abel の retrain インシデント（`20260210-report-retrain-unfiltered-scraping-incident.md`）も分析対象に含め、RFC 品質問題と合わせてより広い視点から問題の構造を考察した。
...
AI は `result-set-register` の挙動を正しくコードから理解できたはずだが、**引数なしで外部スクレイピングを呼び出した場合の運用上の帰結を想像しなかった**。
...
| 層 | 内容 | 自然な置き場所 | 特性 |
|---|---|---|---|
| **行動原則** | プロジェクト固有の制約・禁止事項・注意点 | CLAUDE.md（プロジェクト固有セクション） | AI が理由を理解しなくても従うべきルール。「何をすべきか / してはいけないか」 |
...
| AI の行動を制約するルールか？ | → CLAUDE.md に書く | - |

具体的な線引き:

```
✅ 書く: 「中央競馬（JRA）のみ対象。土日開催のみ。」        → ドメイン知識
✅ 書く: 「purchases/ は IPAT 連携を担う」                   → 構造的オリエンテーション
✅ 書く: 「ML パイプラインは Python (LightGBM)」             → 技術選定（安定的）
❌ 除く: 「purchase-ipat は watchSpanTime 分ごとにポーリング」→ 技術的事実
❌ 除く: 「ipat.dev: true で購入をスキップ」                  → 技術的事実
```
...
- **ドメイン知識の欠如**（RFC 5-1 の #3〜#6）は、docs にドメイン知識が書かれていなければ防げない。これは docs の内容充実の問題であり、情報アーキテクチャの変更だけでは対処できない。
- **AI コーディング時代における人間のコード把握**: docs から技術的事実を除くと、人間が docs を通読してシステム概要を把握するユースケースの有用性は低下する。ただし、このユースケース自体が「AI に聞く」に移行しつつある可能性がある。
- **エンタープライズ規模での適用**: コード調査のコストは RFC のスコープに比例し、コードベース全体には比例しない。構造的オリエンテーション（architecture.md）が「どこを見ればいいか」を提供するため、規模への依存は限定的と考えるが、実証は今後の課題である。
...
| 3 | abel | CLAUDE.md にプロジェクト固有の行動原則を追加 | `CLAUDE.md` |
| 4 | abel | ops.md をドメイン知識中心に再編（技術的事実の記述を除去） | `docs/ops.md` |
| 5 | vdev | csync がプロジェクト固有ルールと共存する方式を検討 | `bin/csync` 関連 |

## 6. 未決事項

- 改善アクション #1〜#5 の優先順位と着手順序
- csync とプロジェクト固有 CLAUDE.md の共存方式の具体設計

...
|---|------|-----------------------------|
| G1 | {達成すべき具体的な目標} | {実装完了後に達成を検証する具体的手順。「記載があること」のような実装と同義の確認は不可。コマンド実行結果、テスト通過、動作確認手順など、実装とは独立した検証手段を記述すること。/vfy で全て実施される。副作用を伴う操作は人間の承認を得て実行してよいが、実施せず人間に委ねることは不可} |
| G2 | ... | ... |

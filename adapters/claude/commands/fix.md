...
1. `--merge` オプションの有無を判定する。入力テキスト内に `--merge` が含まれていればフラグを ON にし、`--merge` を除去した残りを元ネタ文章として扱う。
2. 元ネタ文章が空の場合は、「修正内容を入力してください。」とだけ表示し、ユーザの次のメッセージを待て。

### Phase 1: RFC作成

以下のプロンプトで Task を起動し、RFCを作成させよ。

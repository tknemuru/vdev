...
### Step 2: 仕様書の読み込みと前提条件の検証

1. 仕様書ファイルを Read ツールで読み込む。ファイルが存在しない場合はエラーを報告して終了する。

2. **§4.1「必要な環境情報」の検証**（テーブルが存在する場合）:
   - テーブルの各行から「物理名」と「設定先」を抽出する
   - 各「設定先」ファイルが仕様書と同じリポジトリ内に存在することを確認する
   - 各「物理名」のキーが設定ファイル内に存在し、値がプレースホルダーでないことを確認する
   - プレースホルダー禁止パターン: 空文字、`TODO`、`xxx`、`dummy`、`mock`、`example`、`your-*-here`、`change-me`、`FIXME`（大文字小文字不問）
   - 1件でも FAIL がある場合、以下を出力して**終了する**（Step 3 に進まない）:

   ```
   前提条件エラー: 必要な環境情報が未設定です。

   | 物理名 | 設定先 | 状態 |
   |--------|--------|------|
   | {物理名} | {設定先} | ✗ ファイルが存在しない / ✗ 値がプレースホルダー |

   上記を解決してから /adev を再実行してください。
   ```

3. **§4.2「事前の人間タスク」の検証**（チェックリストが存在する場合）:
   - `- [ ]`（未チェック）と `- [x]`（チェック済み）をパースする
   - 未チェック項目が1件でもある場合、以下を出力して**終了する**:

   ```
   前提条件エラー: 事前の人間タスクが未完了です。

   未完了タスク:
   - [ ] {タスク内容}

   仕様書上でチェックを入れてから /adev を再実行してください。
   ```

4. 指定セクション（デフォルト "4.3"）からRFC一覧テーブルを読み取り、各エントリの以下を抽出する:
   - slug: RFCの識別子
   - scope: スコープまたは概要
